            ...
            >
              {["TRUE", "FALSE", "NOT GIVEN"].map((option) => (
                <button
                  key={option}
                  onClick={() =>
                    !showResults && handleAnswerChange(qIndex, option)
                  }
                  disabled={showResults}
                  style={{
                    padding: "10px 20px",
                    ...
                    fontWeight:
                      currentAnswers[qIndex] === option ? "600" : "400",
                    transition: "all 0.2s",
                  }}
                >
                  {option}
          ...
          <div key={qIndex} style={questionStyle}>
            ...
            {question.matchingPairs?.map((pair, pairIndex) => (
              ...
              >
                ...
                <span style={{ color: isDarkMode ? "#4a90d9" : "#667eea" }}>
                  →
                </span>
                <select
                  value={currentAnswers[`${qIndex}-${pairIndex}`] || ""}
                  onChange={(e) =>
                    handleAnswerChange(`${qIndex}-${pairIndex}`, e.target.value)
                  }
                  disabled={showResults}
                  style={{
                    ...inputStyle,
                ...
                >
                  ...
                  {question.headings?.map((heading, hIndex) => (
                    <option key={hIndex} value={heading}>
                      {heading}
                    </option>
                  ))}
  ...
  return (
    <div style={modalStyle} onClick={onClose}>
      <div style={contentStyle} onClick={(e) => e.stopPropagation()}>
        ...
        >
          <span>
            💡 Đây là chế độ xem trước - học sinh sẽ thấy giao diện tương tự khi
            làm bài
          </span>
          <span>
