...
const TakeListeningTest = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch(apiPath(`listening-tests/${testId}`));
        ...
        if (data.passages[0].audioFile) {
          setAudioUrl(hostPath(data.passages[0].audioFile));
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (test && currentPartIndex < test.passages.length) {
      ...
      if (passage.audioFile) {
        setAudioUrl(hostPath(passage.audioFile));
      }
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(apiPath("listening-submissions"), {
        method: "POST",
        headers: {
          ...
          {currentPart.questions.map((question, qIndex) => (
            <div
              ...
              style={{
                ...
                paddingBottom: "20px",
                borderBottom:
                  qIndex < currentPart.questions.length - 1
                    ? "1px solid #ddd"
                    : "none",
              }}
            >
                    ...
                    question.options.map((option, optIndex) => (
                      ...
                      >
                        <input
                          ...
                          value={option}
                          checked={
                            answers[`${currentPartIndex}_${qIndex}`] === option
                          }
                          onChange={(e) =>
                            handleAnswerChange(
                              currentPartIndex,
              ...
              {question.questionType === "fill" && (
                <input
                  ...
                  onChange={(e) =>
                    handleAnswerChange(currentPartIndex, qIndex, e.target.value)
                  }
                    ...
                    question.options.map((option, optIndex) => (
                      ...
                      >
                        <input
                          ...
                          value={option}
                          checked={(
                            answers[`${currentPartIndex}_${qIndex}`] || ""
                          ).includes(option)}
                          onChange={(e) => {
                            const current =
                              answers[`${currentPartIndex}_${qIndex}`] || "";
        ...
        >
          ...
          <button
            ...
            style={{
              padding: "10px 16px",
              backgroundColor:
                currentPartIndex === test.passages.length - 1
                  ? "#ccc"
                  : "#0e276f",
              color: "white",
              border: "none",
