...
const CreateListeningTest = () => {
  ...
  const handleRemovePassage = (pIndex) => {
    setPassages((prev) => {
      ...
      setActionHistory((h) => [
        ...h,
        { type: "removePassage", pIndex, passage: removed },
      ]);
      const next = prev
        .filter((_, i) => i !== pIndex)
        .map((p, i) => ({ ...p, title: `Part ${i + 1}` }));
      return next;
    });
  };
  ...
  const handleAddQuestion = (pIndex) => {
    setPassages((prev) => {
      const next = [...prev];
      const q = {
        questionType: next[pIndex].partType || "fill",
        questionText: "",
        options: [],
        correctAnswer: "",
      };
      next[pIndex] = {
        ...next[pIndex],
        questions: [...next[pIndex].questions, q],
      };
  ...
  const handleRemoveQuestion = (pIndex, qIndex) => {
    setPassages((prev) => {
      ...
      next[pIndex] = {
        ...next[pIndex],
        questions: next[pIndex].questions.filter((_, i) => i !== qIndex),
      };
      setActionHistory((h) => [
        ...h,
        { type: "removeQuestion", pIndex, qIndex, question: removed },
      ]);
      return next;
    });
  };
  ...
  const handleReplaceQuestion = (pIndex, qIndex, updatedQuestion) => {
    setPassages((prev) => {
      ...
      next[pIndex] = {
        ...next[pIndex],
        questions: next[pIndex].questions.map((qq, i) =>
          i === qIndex ? updatedQuestion : qq
        ),
      };
      return next;
    });
  ...
  const handleRemoveOptionFromQuestion = (pIndex, qIndex) => {
    setPassages((prev) => {
      ...
      setActionHistory((h) => [
        ...h,
        { type: "removeOption", pIndex, qIndex, value: removed },
      ]);
      return next;
    });
  };
    ...
    setActionHistory((prev) => {
      ...
      setPassages((current) => {
        ...
        if (last.type === "addPassage") {
          ...
          const arr = [
            ...next.slice(0, last.pIndex),
            last.passage,
            ...next.slice(last.pIndex),
          ];
          ...
          p.questions = p.questions.map((qq, i) =>
            i === last.qIndex ? q : qq
          );
          next[last.pIndex] = p;
  ...
  const handleSubmit = (e) => {
    ...
    if (!audioFile && !hasPassageAudio) {
      setMessage(
        "❌ Vui lòng tải lên ít nhất một file audio cho bài thi hoặc cho mỗi phần."
      );
      return;
    }
    setIsReviewing(true);
  ...
  const handleConfirmCreate = async () => {
    try {
      ...
      if (audioFile && audioFile.size > MAX_FILE_SIZE) {
        throw new Error(
          `❌ File audio chung quá lớn (>${(
            MAX_FILE_SIZE /
            1024 /
            1024
          ).toFixed(0)}MB). Vui lòng chọn file nhỏ hơn.`
        );
      }
      ...
      const cleanedPassages = passages.map((p) => ({
        ...
        questions: p.questions.map((q) => ({
          ...
          options: q.options
            ? q.options.map((opt) => stripHtml(opt))
            : undefined,
        })),
      }));
      ...
      const res = await fetch(apiPath("listening-tests"), {
        method: "POST",
        body: formData,
      });
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: "20px" }}>
            ...
            <div>
              <small>
                Hoặc bạn có thể tải audio cho từng phần trực tiếp trong từng
                phần (Passage) bên dưới.
              </small>
            </div>
          </div>
                ...
                {passages.map((p, pIndex) => {
                  ...
                  const startFrom =
                    passages
                      .slice(0, pIndex)
                      .reduce(
                        (acc, cur) => acc + (cur.questions?.length || 0),
                        0
                      ) + 1;
                  return (
                    <div key={pIndex} style={{ marginBottom: "26px" }}>
                      ...
                      <div style={{ marginBottom: "10px" }}>
                        <input
                          ...
                          value={p.title}
                          onChange={(e) =>
                            handlePassageChange(pIndex, "title", e.target.value)
                          }
                          style={inputStyle}
                        />
                      </div>
                      ...
                      <div style={{ marginBottom: "10px" }}>
                        ...
                        <input
                          ...
                          accept="audio/*"
                          onChange={(e) =>
                            handlePassageAudioChange(pIndex, e.target.files[0])
                          }
                        />
                        {p.audioFile && (
                          <audio
                      ...
                      <ListeningPart
                        ...
                        onTypeChange={(newType) =>
                          handlePassageChange(pIndex, "partType", newType)
                        }
                        onInstructionChange={(content) =>
                          handlePassageChange(pIndex, "instruction", content)
                        }
                        onQuestionChange={(index, updatedQuestion) =>
                          handleReplaceQuestion(pIndex, index, updatedQuestion)
                        }
                        onAddOption={(qIndex) =>
                          handleAddOptionToQuestion(pIndex, qIndex)
                        }
                        onRemoveOption={(qIndex) =>
                          handleRemoveOptionFromQuestion(pIndex, qIndex)
                        }
                        onRemoveQuestion={(qIndex) =>
                          handleRemoveQuestion(pIndex, qIndex)
                        }
            ...
            <div className="review-modal">
              ...
              <div className="review-modal-body">
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <br />
                  <strong>Người ra đề:</strong> {teacherName || "—"}
                  <br />
                  <strong>Audio chung:</strong>{" "}
                  {audioFile ? audioFile.name : "Không có"}
                  <br />
                </div>
