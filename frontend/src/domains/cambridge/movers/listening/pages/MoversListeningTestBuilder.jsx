...
import {
  ...
  ImageTickEditor,
  ColourWriteEditor,
} from "../../../shared/components/MoversListeningEditorComponents";
...
const normalizeMoversParts = (parts) => {
  ...
  return parts.map((part, partIdx) => ({
    ...part,
    audioUrl: normalizeAudioReference(part?.audioUrl || ""),
...
const MoversListeningTestBuilder = ({ editId = null, initialData = null }) => {
  ...
  const [parts, setParts] = useState(() => {
    if (initialData?.parts && Array.isArray(initialData.parts) && initialData.parts.length === 5) {
      return normalizeMoversParts(normalizeListeningPartsAudio(initialData.parts));
    }
  ...
  const handleAudioUpload = async (file, isGlobal) => {
    ...
    try {
      ...
      if (isGlobal) {
        setMainAudioUrl(normalizedUrl);
      } else {
        updatePart("audioUrl", normalizedUrl);
      }
    } catch (err) {
  ...
  const handleSubmit = async (status = "published") => {
    ...
    try {
      const payload = {
        ...
        mainAudioUrl: normalizeAudioReference(mainAudioUrl) || null,
        parts: normalizeMoversParts(normalizeListeningPartsAudio(parts)),
        totalQuestions: 25,
        status,
      };
          ...
          <div>
            ...
            {mainAudioUrl ? (
              <AudioPreviewBlock
                ...
                rootStyle={{ marginTop: "10px", display: "flex", flexDirection: "column", gap: "10px" }}
                audioStyle={{ width: "100%" }}
                linkStyle={{ color: "#2563eb", textDecoration: "none", fontSize: "13px" }}
              />
            ) : null}
          </div>
        </div>
