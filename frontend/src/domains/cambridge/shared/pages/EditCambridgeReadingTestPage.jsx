...
import CambridgeTestBuilder from "../components/CambridgeTestBuilder";
...
const EditCambridgeReadingTestPage = ({ apiBasePath = 'cambridge/reading-tests' }) => {
	const { id } = useParams();
	...
	useEffect(() => {
		const fetchTest = async () => {
			try {
				setLoading(true);
				const res = await fetch(apiPath(`${apiBasePath}/${id}`));
	...
	return <CambridgeTestBuilder testType={testType} editId={id} initialData={fetchedData} apiBasePath={apiBasePath} />;
};

export default EditCambridgeReadingTestPage;